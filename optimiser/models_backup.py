...
class TransformerOneShot(nn.Module):
    def __init__(self, seq_len, pred_len, in_dim, dropout_rate=0.1):
        super().__init__()
        # Use all features flattened: (B, seq_len * in_dim) -> (B, pred_len)
        self.linear = nn.Linear(seq_len * in_dim, pred_len)
...
def get_model(model_name: str, config: dict):
    ...
    elif model_name == "transformer_oneshot":
        return TransformerOneShot(
            seq_len=config["seq_len"],
            pred_len=config["pred_len"],
            in_dim=config["in_dim"]
        )
    ...
    def __init__(self, seq_len, exog_dim, horizon, d_model=64, n_experts=4, grid_size=3, num_layers=1):
        ...
        for _ in range(self.num_vars):
            if num_layers == 1:
                ...
                branch = MMKLayer(seq_len, horizon, n_experts=n_experts, grid_size=grid_size)
            else:
                layers = []
                layers.append(MMKLayer(seq_len, d_model, n_experts=n_experts, grid_size=grid_size))
                for _ in range(num_layers - 2):
                    layers.append(MMKLayer(d_model, d_model, n_experts=n_experts, grid_size=grid_size))
                layers.append(MMKLayer(d_model, horizon, n_experts=n_experts, grid_size=grid_size))
                branch = nn.Sequential(*layers)
            self.branches.append(branch)
        
    def forward(self, x_ex, y_p, return_internals=False):
        # x_ex: (B, L, exog_dim)
        # y_p: (B, L, 1) already from data_loader separated mode
        ...
        for i in range(self.num_vars):
            ...
            feat_out = self.branches[i](feat_input) # (B, horizon)
        ...
        if return_internals:
            return final_output, {"branch_outputs": stacked_outputs}
        return final_output
